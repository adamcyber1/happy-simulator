...
class ConcurrencyLimitedServer(Server):
    ...
    def start_request(self, request: Event) -> list[Event]:
        ...
        print(f"[{request.time.to_seconds()}][{self.name}][{request.name}] Recomputed latency! Baseline: {self._baseline_latency._mean_latency}Mean is: {self._latency._mean_latency} because concurrency is {self._concurrent_requests}")
